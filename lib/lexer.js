...
const isPlainDiff = d => !d.includes(' ') && !d.includes('/') && escape(d) === d
const keywords = withKeywords({
	licensePlate: licensePlates,
	// keywords don't support spaces
	differentiator: diffs.filter(isPlainDiff),
})
...
const lexer = compile({
	...
	differentiator: {
		// todo: escape them
		match: new RegExp(`${beforeWStart}(?:${diffs.filter(d => !isPlainDiff(d)).map(escape).join('|')})${afterWEnd}`, 'u')
	},
	lParen: '(', rParen: ')',
	lBracket: '[', rBracket: ']',
	// todo: does the `^` anchor work?
	sbahnUbahn: /^[sS]\+[uU](?=\s)/u,
	sbahn: /^[sS](?=\s)/u,
	ubahn: /^[uU](?=\s)/u,
	ubahnLine: {
		match: new RegExp(`${beforeWStart}[uU][0-9]{1,2}${after}`, 'u'),
		type: keywords,
	},
	sbahnLine: {
		match: new RegExp(`${beforeWStart}[sS][0-9]{1,2}X?${after}`, 'u'),
		type: keywords,
	},
	station: {
